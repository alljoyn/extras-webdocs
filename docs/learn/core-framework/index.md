...
- (TODO: add video)
- (TODO: should I talk about bus attachment here?)
...
a service for an stovetop, the StoveBurner BusObject can be attached to
...
A Sessionless Signal is a mechanism to receive signals without having to
...
Events and Actions is a convention for applications to describe its
...
An AllJoyn Application interacts with the AllJoyn framework via the Bus Attachment.
The application advertises its services via the About Announcement,
which lists out metadata about the application, including the supported
interfaces. The UniqueName is returned in discovery to identify the 
application.
...
properties, receiving signals (sending?).

![alljoyn-core-components][alljoyn-core-components]

## Under the Hood

- In the future, add a summary of how the Alljoyn framework works under the hood, for now, link to Binita's tome

[about]: /learn/core-framework/about-announcement
