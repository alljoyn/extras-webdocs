...
the scons variable, this method is intended for debug and test purposes.
...
The standalone RN runs as the compiled application `alljoyn-daemon`.  If the
user does not specify a separate configuration file when alljoyn-daemon
is started, or the `--internal` switch is used, then the RN will use a
configuration that is hardcoded in the source.

**Example:** start routing node using internal config
...
**Standalone RN hardcoded configuration:**
```xml
<busconfig>
    <type>alljoyn</type>
...
As previously mentioned, a standalone RN will accept an external config file
while a bundled RN will not.  In order to start the standalone RN with an
external config file, enter the following command:
...
`max_remote_clients_tcp` and `max_remote_clients_udp` (i.e. `<limit>` elements).
...
|ns_disable_directed broadcast   |disable subnet broadcast if true|false|Used for debugging.  Do not use in deployed production applications.|
...
The config file setting for `<fork>` may be overridden from the
...
|session_setup_timeout|The maximum amount of time that incoming connections are allowed to setup session routes. Beyond this limit, incoming connections may be disconnected.|30000 (ms)|
...
### ``<listen>``
Identifies an address on  which the bus should listen for incoming connections.
This address must be a valid URI that begins with an AllJoyn transport name,
followed by appropriate parameters and options.
...
#### URI Format for AllJoyn Transports

|Transport|Description|Example|Notes|
|------------------------------------|-------------------|
|tcp|AllJoyn over TCP|``<listen>tcp:iface=*,port=9955</listen>``<br>``<listen>tcp:addr=0.0.0.0,port=9955</listen>``<br>`<listen>tcp:addr=10.4.108.34,port=9955</listen>`<br>`<listen>tcp:iface=eth0,port=9955</listen>`<br>`<listen>tcp:iface=*,port=0</listen>`<br><br>*Note:* `port=0` indicates that the transport will use an ephemeral port.| |
|udp|AllJoyn over UDP|``<listen>udp:iface=*,port=9955</listen>``<br>``<listen>udp:addr=0.0.0.0,port=9955</listen>``<br>`<listen>udp:addr=10.4.108.34,port=9955</listen>`<br>`<listen>udp:iface=eth0,port=9955</listen>`<br>`<listen>udp:iface=*,port=0</listen>`<br><br>*Note:* `port=0` indicates that the transport will use an ephemeral port.| |
|launchd|Used only for IOS and OS X.|``<listen>launchd:env=DBUS_LAUNCHD_SESSION_BUS_SOCKET</listen>``| |
|unix|Unix domain socket.  Applicable only for standalone routing nodes on posix platforms.|On Linux:<br>``<listen>unix:abstract=[some name]</listen>``<br><br>For example:<br>`<listen>unix:abstract=alljoyn-1</listen>`|`unix:abstract=` is used only with Linux; however, all POSIX systems may use `unix:path=`. Note that the specified path must refer to an existing named socket. Also, `unix:abstract=alljoyn` is the default because applications look for the "alljoyn" socket name by default.
|slap|Serial Line over AllJoyn Protocol.  Used to provide an interface between an AllJoyn embedded platform and a POSIX-based routing node.|`<listen>slap:type=uart,dev=/dev/ttyUSB0,baud=115200</listen>`<br><br>*Note:*  `dev` is POSIX platform specifc and `baud` is configurable.| |
