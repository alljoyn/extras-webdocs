...
external config file, enter the following command (Linux):
...
|--------------------------------|
|iface|Valid network interface on the device that is hosting the RN.|Yes|`iface` and `addr` are mutually exclusive<br>`iface` is preferred over `addr`|
|addr|Valid IPv4 address|No|`iface` and `addr` are mutually exclusive<br>`addr` primarily exists for backward compatibility.  Prefer `iface`, instead.|
|port|Valid TCP or UDP port number|No| |
|abstract|Linux named socket|No|Linux only.<br>`unix:abstract=alljoyn` is the default because applications look for the "alljoyn" socket name by default|
|path|POSIX named socket| |The specified path must refer to an existing named socket.|
